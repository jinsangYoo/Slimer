...
var _AceTM = (function (oa) {
	console.log("###1 ACE AceCoutner Start ###");
	var _t = {
		u: 'unknown',
		d: 'undefined',
		n: 'null',
	...
	var _f = {
		...
		getBundleID:function(){
			var successCallback=function(result){
				console.log("### success get BundleID"+result);
				_f.bundleid = result;
			};
			var failureCallback=function(result){
				console.log("### failure get BundleID"+result);
			};
			if(typeof(AceSDK)!=_t.d){
				console.log("### Script Get BundleID Start");
				AceSDK.getBundleID(successCallback, failureCallback);
			};
		},
		getBundlePath:function(){
			var successCallback=function(result){
				console.log("### success get BundlePath"+result);
				_f.bundlepath = result;
			};
			var failureCallback=function(result){
				console.log("### failure get BundlePath"+result);
			};
			if(typeof(AceSDK)!=_t.d){
				console.log("### Script Get BundlePath Start");
				AceSDK.getBundlePath(successCallback, failureCallback);
			};
		},
		getQuery:function(name,url){
			console.log("find queryString:"+url);
			if(!url) return;
			name = name.replace(/[\[\]]/g, '\\$&');
		...
		tpc : function(val){
			console.log('### Start push JSON parse:'+val);
			var type = typeof(val);var rep = '';var ret = '';
			switch(type){
				case "string":
					if(typeof(JSON)!=_t.d){
						rep = JSON.parse(val);
			...
			return rep.additionalData.acp_push;
		},
		pvSend : function(parameter){

			console.log("### ACESDK check : "+typeof(AceSDK));
			var success = function(result) {
			    console.log("send try pv::" + result);
			}
			var failure = function(result) {
			    console.log("failure send::" + result);
			}
			try{
				console.log("Call PV");
				console.log('AceSDK Type Check :'+typeof(AceSDK));
					AceSDK.pv(parameter,success,failure);
				console.log("Call End");
			}catch(e){
				console.log("Send try error :"+e);
			}
		},
		getSDKparam : function(w,str){
			console.log("### ACESDK check : "+typeof(AceSDK));
			var success = function(result) {
			    console.log("send try getSDKParam::" + result);
					console.log('send try str:'+str);
					_f.va = result;
					if(str!=''){
						_p.sendionic(w,str);
					}else{
			...
			var failure = function(result) {
			    console.log("failure getSDKParam::" + result);
			}
			try{
				console.log('AceSDK Type Check :'+typeof(AceSDK));
				if(typeof(AceSDK)!=_t.d){
					AceSDK.getPVParameter("", success, failure);
				}else{
				...
				console.log('### getSDKparam Error : '+e);
			}
		},
		getServiceId :  function(){
			console.log("### ACESDK check : "+typeof(AceSDK));
			var success = function(result) {
			    console.log("send try getServiceId::" + result);
				_f.rr = result;
			}
			var failure = function(result) {
			    console.log("failure getServiceId::" + result);
			}
			try{
				console.log('AceSDK Type Check :'+typeof(AceSDK));
				// setTimeout(function() {
					AceSDK.getServiceId(success, failure);
				// }, 500);
			}catch(e){
				console.log('### ACE getService ID Error : '+e);
			}
		...
		pvDummy : function(){
			console.log("### ACESDK check : "+typeof(AceSDK));
			var success = function(result) {
			    console.log("send try pvDummy::" + result);
			}
			var failure = function(result) {
			    console.log("failure pvDummy::" + result);
			}
			try{
				console.log('AceSDK Type Check :'+typeof(AceSDK));
				// setTimeout(function() {
					AceSDK.pvDummy("js_test", success, failure);
				// }, 500);
			}catch(e){
				console.log('### ACE pvDummy Error : '+e);
			}
		},
		setDeeplinkHeader:function(gsck){
			console.log("### ACESDK check : "+typeof(AceSDK));
			var success = function(result){
				//...
				console.log('### setGsck Success:'+result);
			}
			var failure = function(result){
				//..
				console.log('### setGsck failure:'+_f.getDeeplinkHeader());
			}
			try{
				console.log('AceSDK Type Check :'+typeof(AceSDK));
				//document.addEventListener("deviceready", function() {
					AceSDK.setGsck(gsck,success,failure);
				//});
			}catch(e){
				console.log('### ACE setHeader Error:'+e);
			}
		},
		getDeeplinkHeader:function(){
			console.log("### ACESDK check : "+typeof(AceSDK));
			var success = function(result){
				//...
				console.log('### getGsck Success:'+result);
			}
			var failure = function(result){
				//..
				console.log('### getGsck failure:'+result);
			}
			try{
				console.log('AceSDK Type Check :'+typeof(AceSDK));
				AceSDK.getGsck(success,failure);
			}catch(e){
				console.log('### ACE getHeader Error:'+e);
			}
		},
		al: '',
		...
		cid: function () {
			...
			var _rnc = _f.rnd();
			console.log('### ACE CID:'+ String(oa.em + '' + _rnc).substr(0, 19));
			var hexStr = String('25123').replace(/[^a-f0-9]/gi, '');
			return String(oa.em + '' + _rnc).substr(0, 19);
		},
		uqi: function () {
			var n = _f.cid();
	...
	var _p = {
		...
		p: (location.protocol.indexOf('https') > -1 ? location.protocol : "http:") + '//dnt.acecounter.com',
		all: function (ret, ex) {
			var toS = '';
			for (var _ix in ret) {
		...
		send:function (w, s, x, c) {

			var _aceHybridApp = typeof(window.aceHybridApp) != _t.d ? window.aceHybridApp : '';
			var an = _sse.ast(c);
			var buy = _def.buy(oa);
			var sm = {
				aci: oa.acid,
			...
			if(_aceHybridApp==4){
				_f.getServiceId();
				console.log("### ACE Plugin ssid Start : "+_f.rr);
				sm.aci = _f.rr;
				console.log("### Ace Plugin ServiceId Set :"+sm.aci);
			}
			console.log("###3 ACE sid Check : "+sm.aci);
			var em = {
				rel: 0,
				url: '',
				prmt: '',
			...
			if(_aceHybridApp==4){
				if(typeof(s)!=_t.d){
					console.log('### ACE Fixed URL : '+typeof(s)+'@'+JSON.stringify(s));
					_f.getSDKparam(w,s);
					return;
				}else{
					_f.getSDKparam(w);
			...
			console.log("### ACE Plugin getSDKParam :"+_L);
			if (_f.ap.length == 6 && _L != '') {

				console.log("###3-1 ACE from SDK Parameter : "+_L);
				sm.aci = _f.gpr(_L, 'aci', oa.acid);
				_f.ref = _f.gpr(_L, 'eref', _f.ref);
				_f.ref = _f.u(_f.ref);
				_f.url = _f.gpr(_L, 'url', _f.url);
				_f.url = _f.u(_f.url);
				console.log("### ACE SDK URL Set :"+_f.url);

				an.avt = _f.gpr(_L, 'avt', an.avt);
				an.avn = _f.gpr(_L, 'avn', an.avn);
				if (an.avn == 0) {
					if (document.referrer != '' || document.referrer != 'bookmark') {
						_f.ref = document.referrer;
					}
				} else if (an.avn == 1) {
					_f.al = _L.replace('avn=1', 'avn=0');
				}
			...
			if(_aceHybridApp == 4){
				_f.url = _f.u(location.pathname);
				console.log("### ACE SDK URL Set :"+_f.url);
			}
			console.log("### ACE SDK PCM param : "+_s.pcm);
			var stm = _s.toStr();
			var up = _k.get();

			if (_f.ap.length == 6 && _f.al != '') {
				em.prmt = _f.gpr(_L, 'prmt', _f.e(up.pam));
				em.tk=_f.gpr(_L, 'tk', _f.e(up.pam));
				console.log('### ACE RECIEVE tk : '+em.tk);
			} else {
				em.prmt = _f.e(up.pam);
				em.tk='';
			}
			...
			if (typeof(w) != _t.d) {
				...
				console.log("###4 ACE aceHybridApp Variable Check : "+_aceHybridApp);

				switch (_aceHybridApp) {
				case 1:
				console.log("###5 ACE Send Adnroid SDK Parameter : "+_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					oa.sendNativeLog(_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					break;
				case 2:
				console.log("###5 ACE Send iOS UIWebView SDK Parameter : "+_p.all(sm).replace('&aci', '?aci') + _p.all(an) + _p.all(em) + stm + slog);
					document.location = "acescheme://scriptLogging" + _p.all(sm).replace('&aci', '?aci') + _p.all(an) + _p.all(em) + stm + slog;
					break;
				case 3:
				console.log("###5 ACE Send iOS WKWebView SDK Paramater : "+_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					webkit.messageHandlers.acescheme.postMessage(_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					break;
				case 4:
				console.log("###5 ACE Send iOS Plugin SDK Parameter : "+_p.all(sm).replace('&aci', 'aci') + _p.all(an) + _p.all(em) + stm + slog);
					var param = _p.all(sm).replace('&aci', 'aci') + _p.all(an) + _p.all(em) + stm + slog;
					try{
						_f.pvSend(param);
					}catch(e){
						console.log("### ACE iOS PV Error : "+e);
					}
					break;
				default:
				console.log("###5 ACE Send Collector by Script : "+_p.all(sm).replace('&aci', 'aci') + _p.all(an) + _p.all(em) + stm + slog);
					_p.Img(_p.all(sm).replace('&aci', 'aci') + _p.all(an) + _p.all(em) + stm + slog);
					break;
				}
		...
		sendionic:function(w,s,x,c){
			...
			if(_aceHybridApp==4){
				_f.getServiceId();
				console.log("### ACE Plugin sendionic ssid Start : "+_f.rr);
				sm.aci = _f.rr;
				console.log("### Ace Plugin sendionic ServiceId Set :"+sm.aci);
			}
			console.log("###3 ACE sendionic sid Check : "+sm.aci);
			var em = {
				rel: 0,
				url: '',
				prmt: '',
			...
			if (_f.ap.length == 6 && _L != '') {
				...
				console.log("###3-1 ACE from SDK sendionic Parameter : "+_L);
				sm.aci = _f.gpr(_L, 'aci', oa.acid);
				_f.ref = _f.gpr(_L, 'eref', _f.ref);
				_f.ref = _f.u(_f.ref);
				_f.url = _f.gpr(_L, 'url', _f.url);
				_f.url = _f.u(_f.url);
				console.log("### ACE SDK sendionic URL Set :"+_f.url);
			...
			_f.url = _f.u(location.pathname);
			console.log("### ACE SDK sendionic URL pathname Set :"+_f.url);

			console.log("### ACE SDK sendionic PCM param : "+_s.pcm);
			var stm = _s.toStr();
			var up = _k.get();
			console.log('### adqwoidjeoqiwdjhfoiwejroiwejfoiwjrfoij'+_L);
			if (_f.ap.length == 6 && _L != '') {
				console.log('### Check TK : '+_f.tk+'@'+_f.as+'@'+_f.ms);
				em.prmt = _f.gpr(_L, 'prmt', _f.e(up.pam));
				em.tk= _f.gpr(_L, 'tk', _f.tk);
				em.as= _f.gpr(_L, 'as', _f.as);
				em.ms= _f.gpr(_L, 'ms', _f.ms);
				em.referrer= _f.gpr(_L, 'referrer', _f.referrer);
			} else {
				em.prmt = _f.e(up.pam);
				em.tk='';
			}
			console.log('### push string:'+_f.pStr);
			if(_f.pStr!=''){
				em.prmt = _f.e(_f.pStr);
				em.rdm = "Direct";
			}

			_f.getBundleID();
			em.url = _f.e(_f.hr(up.url, 'u'));
			em.eref = _f.e(_f.hr(_f.bundleid+up.ref, 'r'));
			console.log('### ACE SDK sendionic url set before : '+em.url);
			//em.url = _f.e(_f.hr(up.url, 'u'));
			em.url = _f.e(_f.hr(up.url, 'u'));
			console.log('### ACE SDK sendionic url set after : '+em.url);
			//em.eref = _f.e(_f.hr(up.ref, 'r'));
			em.eref = _f.e(_f.hr(_f.bundleid+up.ref, 'r'));
			console.log('### ACE SDK sendionic eref set after : '+em.eref);
			if (typeof(w) != _t.d) {
				switch (w) {
				case "SITE":
					var udef = _def.ud(oa);
					...
					if (typeof(s) != _t.d) {
						if(String(s)!=''){
							console.log('### ACE Second Arg:'+JSON.stringify(s));
							//em.eref = em.url;
							em.eref = _f.bundleid+em.url;
							em.rdm = _f.hr(_f.url, 'h');
							em.url = _f.e(_k.r(s, '?'));
							em.prmt = _f.e(_k.up(s));
						}
					...
					if(_aceHybridApp==4){
						em.rdm = decodeURIComponent(em.rdm);
					}
					if(_f.tk!=''){
						em.rdm="Direct";
					}
				...
				console.log("###4 ACE aceHybridApp sendionic Variable Check : "+_aceHybridApp);

				switch (_aceHybridApp) {
				case 1:
				console.log("###5 ACE Send Adnroid SDK sendionic Parameter : "+_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					oa.sendNativeLog(_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					break;
				case 2:
				console.log("###5 ACE Send iOS UIWebView SDK sendionic Parameter : "+_p.all(sm).replace('&aci', '?aci') + _p.all(an) + _p.all(em) + stm + slog);
					document.location = "acescheme://scriptLogging" + _p.all(sm).replace('&aci', '?aci') + _p.all(an) + _p.all(em) + stm + slog;
					break;
				case 3:
				console.log("###5 ACE Send iOS WKWebView SDK sendionic Paramater : "+_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					webkit.messageHandlers.acescheme.postMessage(_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					break;
				case 4:
				console.log("###5 ACE Send iOS Plugin SDK sendionic Parameter : "+_p.all(sm).replace('&aci', 'aci') + _p.all(an) + _p.all(em) + stm + slog);
					var param = _p.all(sm).replace('&aci', 'aci') + _p.all(an) + _p.all(em) + stm + slog;
					try{
						_f.pvSend(param);
						_f.tk='';
						...
						console.log("### ACE iOS PV sendionic Error : "+e);
					}
					
					
					break;
				default:
				console.log("###5 ACE Send Collector sendionic by Script : "+_p.all(sm) + _p.all(an) + _p.all(em) + stm + slog);
					_p.Img(_p.all(sm).replace('&aci', '?aci') + _p.all(an) + _p.all(em) + stm + slog);
					break;
				}
				if (w == 'SITE' && _f.len(buy.orn) != 0 && em.rel == 0) {
	...
	oa.Data = function (t1, t2, t3, t4) {
		var tStr = '&sk1=' + t1 + '&sk2=' + t2 + '&sk3=' + t3 + '&sk4=' + t4;
		var cu = _f.hr(_f.url, 'h');
		_p.send('DATA', cu, tStr);
	};
	...
	oa.applog = function (a) {
		console.log("###2 ACE SDK Call applog ");
		_f.al = a;
		_p.send("SITE");
		_c.Run();
	};
	...
	b: json Object
	*/
	oa.referPV = function(a,b){
		a=((typeof(a)!=_t.d)?a:'');
		b=((typeof(b)!=_t.d)?b:'');
		//console.log(typeof(b));
		var j='';
		console.log('### ACE Call Start acepushPV : '+a+'::'+b);
		switch(a){
			case 1:
			// todo json parse
			console.log('### ACE push send');
			j=_f.tpc(b);
			console.log(j);
			_f.pStr = String(j);
			if(_f.pStr!=''){
				...
				console.log('### not a push String');
			}
			//Send
			break;
			case 2:
			console.log('### ACE deeplink send');
			console.log('### ACE deeplink Object:'+typeof(b));

				var _Type = typeof(b);
				switch(_Type){
					case "string":
						if(_f.getQuery("tk",b)!=null&&_f.getQuery("gsck",b)!=null){
							_f.tk = _f.getQuery("tk",b);
							_f.setDeeplinkHeader(_f.getQuery("gsck",b));
							_p.send("SITE");
						}else{
							console.log("### can't find tk:"+_f.getQuery("tk",b)+"or gsck:"+_f.getQuery("gsck",b));
						}
					break;
					case "object":
						if(_f.getJSON("$link",b)!=null){
							var j=_f.getJSON("$link",b);
							if(_f.getQuery("tk",j)!=null&&_f.getQuery("gsck",j)!=null){
								_f.tk = _f.getQuery("tk",j);
								_f.setDeeplinkHeader(_f.getQuery("gsck",j));
								_p.send("SITE");
							}else{
								console.log("### can't find tk:"+_f.getQuery("tk",j)+"or gsck:"+_f.getQuery("gsck",j));
							}
						}
					break;
					default:
	...
	console.log('### ACE acid check first :'+oa.acid);
	var aceHybridApp = typeof(window.aceHybridApp) != _t.d ? window.aceHybridApp : '';
	if(aceHybridApp==4){
		var acei=1;

		var cksid = setInterval(function(){
			if(oa.acid == _t.d){
				console.log("## set SID");
				oa.acid = window.name;
				acei++;
		...
		if (typeof(oa.acid) != _t.d) {
			//console.log('### ACE HybridApp : '+window.aceHybridApp);
			if(_f.ap.length != 6){
				console.log("###2 ACE Script SendLog by Web Start acid :"+oa.acid);

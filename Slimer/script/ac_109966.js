var _AceTM = (function(oa) {
  ...
  var hashChangedEvent = function () {
      oa.PV(location.href.replace(document.location.origin, ''));
  };
  var _s1 = {
    rl: function(n, s) {
      var t4 = (n << s) | (n >>> (32 - s));
  ...
  var _def = {
    ...
    ud: function(oo) {
      ...
      var rmv = _def;
      if (typeof(ou) == _t.o) {
        if (_f.vStr(ou.uID, 40) != '') {
          rmv.mid = _f.vStr(ou.uID, 40);
          rmv.mt = 1;
      ...
      if (typeof(w) != _t.d) {
        switch (w) {
          case "SITE":
            ...
            oa.apid = _f.hr(up.url, 'h') + '=' + sm.aci + '-' + an.cid + '-' + an.avt + '-' + an.avc + '-' + an.uid + '-' + an.did + '-' + an.ovt + '-' + an.ck + '-' + _s.pcm;
            break;
          case "NOW":
          case "WISH":
          case "CART":
        ...
        } else if (_aceHybridApp > 1 && _aceHybridApp < 4) {
            window.addEventListener("hashchange", hashChangedEvent, false);
  ...
  oa.PV = function(q) {
    ...
    if ((q != "") && (q.substr(0, 1) == "/")) {
      try {
        window._AceTM_CAET = (new Date().getTime());
      } catch (_e) {}
      cu = (_f.hr(_f.url, 'h') + '' + q);
      cu = cu.replace(/\/\//gi, "/");
      _p.send('SITE', cu);
